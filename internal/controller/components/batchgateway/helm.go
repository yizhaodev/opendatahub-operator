...
func specToHelmValues(gw *componentApi.LLMBatchGateway) (map[string]interface{}, error) {
	...
	if len(gw.Spec.Processor.ModelGateways) > 0 {
		...
		for model, spec := range gw.Spec.Processor.ModelGateways {
			s := spec
			mg[model] = inferenceGatewayToMap(&s)
		}
...
func inferenceGatewayToMap(gw *componentApi.BatchGatewayInferenceGatewaySpec) map[string]interface{} {
...
func apiServerConfigToMap(cfg *componentApi.BatchGatewayAPIServerConfigSpec) map[string]interface{} {
	m := map[string]interface{}{}
...
func mergeProcessorConfig(m map[string]interface{}, cfg *componentApi.BatchGatewayProcessorConfigSpec) {
	if cfg.NumWorkers != 0 {
